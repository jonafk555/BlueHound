...
├── docker-compose.dev.yml      # 開發覆蓋設定（需明確 -f 指定）
...
所有端點預設綁定 `http://localhost:8443`。如設定 `BLUEHOUND_API_KEY`：

- API client 使用 `X-API-Key`。
- 瀏覽器開啟首頁時會顯示 HTTP Basic 登入視窗；username 可任意填寫，password 使用 API key。
- `BLUEHOUND_ENV=production` 未設定 key 時服務會拒絕啟動。
...
| ID | 類別 | 防護措施 |
|----|------|----------|
| VULN-01 | 認證 | API Key / HTTP Basic 驗證；production 缺 key 時 fail closed |
| VULN-03 | 資源管理 | 串流上傳、JSON body/欄位限制、模型輸出 token 上限 |
| VULN-04 | CORS | 限制性 CORS 白名單 (非 `*`) |
| VULN-06 | 注入 | KQL/SPL literal 轉義、Sigma 使用 YAML serializer |
| VULN-07 | 網路 | 預設綁定 localhost (127.0.0.1)，非 0.0.0.0 |
| VULN-10 | XXE | `defusedxml` 防禦 XML 外部實體攻擊 |
| VULN-14 | 供應鏈 | `pip-audit` 依賴套件漏洞掃描 |
| VULN-15 | LLM 安全 | Prompt injection 時跳過模型、結構化 prompt、deterministic severity floor |
| VULN-17 | SSRF | Ollama URL 白名單驗證 |
| VULN-19 | CSP | Content-Security-Policy 標頭、D3 自託管 |
...
| `ALLOW_CLOUD_FALLBACK` | `false` | Ollama 失敗時是否允許將 bounded telemetry 送往 OpenAI |
| `LLM_MAX_OUTPUT_TOKENS` | `1200` | 模型最大輸出 token |
...
| `BLUEHOUND_ENV` | `development` | `production` 模式強制要求 API key |
| `BLUEHOUND_API_KEY` | *(空)* | API 認證金鑰；production 必填 |
| `ALLOW_UNAUTHENTICATED` | `false` | 僅限隔離開發環境，允許無 key 綁定非 loopback |
| `ALLOWED_ORIGINS` | `http://localhost:8443,...` | CORS 允許來源 |
| `BLUEHOUND_MAX_JSON_BODY_BYTES` | `2097152` | JSON API request body 上限 |
...
```bash
PYTHONPATH=backend venv/bin/python -m unittest discover -s tests -p 'test_*.py' -v
